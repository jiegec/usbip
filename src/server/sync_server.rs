        ...
        loop {
            ...
            match command.unwrap() {
                ...
                UsbIpCommand::OpReqImport { busid, .. } => {
                    ...
                    res.write_to_socket(&mut socket).await?;
                ...
                match listener.accept().await {
                    Ok((socket, addr)) => {
                        ...
                        tokio::spawn(async move {
                            match server.handler(socket).await {
                                Ok(()) => {
                                    trace!("Handler for {} ended successfully", addr);
                                }
                                Err(err) => {
                                    warn!("Handler for {} ended with error {:?}", addr, err);
                                }
                            }
                        });
                    }
                    Err(err) => {
                        warn!(
                            "Could not establish socket for incoming connection, {:?}",
                            err
                        );
                    }
                }
...
mod tests {
    ...
    fn new_server_with_single_device() -> SyncUsbIpServer {
        SyncUsbIpServer::new_simulated(vec![UsbDevice::new(0).with_interface(
            ClassCode::CDC as u8,
    ...
    async fn add_and_remove_10_devices() {
        ...
        tokio::spawn(server_.serve(addr));
